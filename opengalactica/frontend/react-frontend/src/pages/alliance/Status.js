      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={{ color: 'white' }}>{isMobile ? 'Origin' : 'Origin'}</TableCell>
          ...
          {Object.entries(grouped).map(([target, group], index) => {
            ...
            return (
              <React.Fragment key={index}>
                {isMobile && (
                  <TableRow>
                    <TableCell colSpan={7} sx={{ color: '#aaa', fontStyle: 'italic' }}>
                      Destination: {target}
                    </TableCell>
                  </TableRow>
                )}
                ...
                {group.map((fleet, i) => (
                  <TableRow
                    ...
                    sx={{
                      backgroundColor:
                        fleet.role === 'Attackers' ? '#b71c1c' : '#1b5e20',
                    }}
                  >
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={{ color: 'white' }}>Arrival</TableCell>
            <TableCell sx={{ color: 'white' }}>Species</TableCell>
            {isMobile ? null : <TableCell sx={{ color: 'white' }}>Destination</TableCell>}
            <TableCell sx={{ color: 'white' }}>Alliance</TableCell>
            <TableCell sx={{ color: 'white' }}>Points</TableCell>
          </TableRow>
        </TableHead>
    ...
    const fetchFleets = async () => {
      try {
        const [incRes, outRes] = await Promise.all([
          api.get('alliance/incoming/'),
          api.get('alliance/outgoing/'),
        ]);
        setIncoming(incRes.data);
        setOutgoing(outRes.data);
      } catch (err) {
        setError('Failed to load alliance fleet data.');
      } finally {
        setLoading(false);
      }
    };
